...
describe('Task e2e test', () => {
  ...
  const taskSample = { title: 'along bandwidth woefully', dueDate: '2026-02-22', completed: true };
  ...
  describe('new Task page', () => {
    ...
    it('should create an instance of Task', () => {
      cy.get(`[data-cy="title"]`).type('across favorable cruelty');
      cy.get(`[data-cy="title"]`).should('have.value', 'across favorable cruelty');

      cy.get(`[data-cy="description"]`).type('../fake-data/blob/hipster.txt');
      cy.get(`[data-cy="description"]`).invoke('val').should('match', new RegExp('../fake-data/blob/hipster.txt'));

      cy.get(`[data-cy="dueDate"]`).type('2026-02-21');
      cy.get(`[data-cy="dueDate"]`).blur();
      cy.get(`[data-cy="dueDate"]`).should('have.value', '2026-02-21');

      cy.get(`[data-cy="completed"]`).should('not.be.checked');
      cy.get(`[data-cy="completed"]`).click();
      cy.get(`[data-cy="completed"]`).should('be.checked');

      cy.get(`[data-cy="completedDate"]`).type('2026-02-22');
      cy.get(`[data-cy="completedDate"]`).blur();
      cy.get(`[data-cy="completedDate"]`).should('have.value', '2026-02-22');

      cy.get(`[data-cy="priority"]`).type('2');
      cy.get(`[data-cy="priority"]`).should('have.value', '2');

      cy.get(`[data-cy="assignedTo"]`).type('pace known');
      cy.get(`[data-cy="assignedTo"]`).should('have.value', 'pace known');
